...
PackedLines::PackedLines(LinesType eType) { m_eType = eType; }

PackedLines::LinesType g3::PackedLines::GetLinesType() const { return m_eType; }
...
bool PackedLines::HasPositions() const { return !m_vPositions.empty(); }
...
void PackedLines::ResizeVertices(unsigned int nVertices, bool bNormals,
                                 bool bColors) {
...
void PackedLines::CopyVertices(const std::vector<float> &vPositions,
...
void PackedLines::CopyVertices(const float *pPositions, size_t nVertices,
                               const float *pColors) {
  m_vPositions.resize(nVertices * 3);
...
void PackedLines::GetSegmentIndices(unsigned int k, unsigned int &i0,
...
void PackedLines::GetSegment(unsigned int k, g3::Vector3f &v0,
                             g3::Vector3f &v1) const {
  unsigned int i0, i1;
