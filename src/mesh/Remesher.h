...
public:
  ...
  enum class SmoothTypes { Uniform, Cotan, MeanValue };
...
protected:
  ...
  virtual ProcessResult ProcessEdge(int edgeID) {
    ...
    EdgeConstraint constraint = (constraints == nullptr)
                                    ? EdgeConstraint::Unconstrained()
                                    : constraints->GetEdgeConstraint(edgeID);
  ...
  abort_collapse:
    ...
    if (EnableFlips && constraint.CanFlip() && bIsBoundaryEdge == false) {
      ...
      bool a_is_boundary_vtx =
          (MeshIsClosed) ? false
                         : (bIsBoundaryEdge || mesh->IsBoundaryVertex(a));
      bool b_is_boundary_vtx =
          (MeshIsClosed) ? false
                         : (bIsBoundaryEdge || mesh->IsBoundaryVertex(b));
  ...
  virtual Vector3d ComputeSmoothedVertexPos(
    ...
    VertexControl vControl = (VertexControlF == nullptr)
                                 ? VertexControl::AllowAll
                                 : VertexControlF(vID);
