...
CMeshHandle create_packed_mesh() { return new MeshHandle(new PackedMesh); }

CMeshHandle create_dynamic_mesh() { return new MeshHandle(new DMesh3()); }
...
g3ExternalC int get_vertices_float(CMeshHandle handle, int buffer_size,
                                   float *buffer) {
  if (is_mesh(handle) == 0)
    return -1;
...
g3ExternalC int get_vertices_double(CMeshHandle handle, int buffer_size,
                                    double *buffer) {
  if (is_mesh(handle) == 0)
    return -1;
...
g3ExternalC int get_triangles(CMeshHandle handle, int buffer_size,
                              unsigned int *buffer) {
  if (is_mesh(handle) == 0)
    return -1;
